...
class RegisterAPIVIEW(generics.GenericAPIView):
    '''
    POST - Stores a user instance in the database allowing a user to register to the service.
    '''
    def __init__(self):
...
class LoginAPIVIEW(generics.GenericAPIView):
    '''
    GET - User can get their missing refresh token to extend their session (DEPRECATED)
    POST - User logs in and is sent back tokens to access the service
...
class UpdateProfile(generics.GenericAPIView):
    '''
    PATCH - User details are updated
    DELETE - User instance is completely removed from the service and database and their tokens blacklisted
    '''
    def __init__(self):
