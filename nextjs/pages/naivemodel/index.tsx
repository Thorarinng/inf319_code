...
const getLikes = async (p: any) => {
  // Gets user by userId
  const likeRes = await axios.get(`api/naive/like/${p.id}`);
...
const getPosts = async () => {
  ...
  const res = await axios.get("api/naive/post/");
...
const getFollowers = async () => {
  ...
  const res = await axios.get("api/naive/follow/followers/");
...
const getFollowing = async () => {
  ...
  const res = await axios.get("api/naive/follow/following/");
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  const token =
    "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ0b2tlbl90eXBlIjoiYWNjZXNzIiwiZXhwIjoxNjYzNjc1NDE3LCJpYXQiOjE2NTE2NzU0MTcsImp0aSI6IjJiMDEzNGRkYzMwOTQzODdiMjE2OGUxZmY0NzQ2OTEzIiwidXNlcl9pZCI6MX0.uIVc5opzOs2FRGZnvG4iYttfD6TURNlJpzvCwM3il_U";
